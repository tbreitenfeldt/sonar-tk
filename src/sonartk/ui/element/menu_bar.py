...
from sonartk.util.key_handler import Key, KeyHandler
...
class MenuBar(Element[MenuBarItem]):
    ...
    def _bind_keys(self) -> None:
        ...
        self.key_handler.add_key_press(self.open_menu, key.SPACE)
        ...
        self.key_handler.add_key_release(self.close, key.LALT)
        self.key_handler.add_key_release(self.close, key.RALT)
    ...
    def setup(  # type: ignore[override]
        ...
        self.state_machine.current_state.setup(
            change_state, interrupt_speech=False
        )
        return True
    ...
    def open_menu(self, key: Optional[str] = None) -> bool:
        ...
        if not self.is_open:
            ...
            self.parent.state_machine.current_state = self
            self.setup(self.parent.state_machine.change)
            # Now that menus are expanded, setup the current menu
            state_key: str = list(self.state_machine.states)[self.position]
    ...
    def close(self) -> bool:
        if self.is_expanded:
            ...
            super().exit()
            self.parent.state_machine.current_state = EmptyState()
            # Delay restoring container state to avoid re-triggering ALT handler
            pyglet.clock.schedule_once(
                lambda dt: self.parent.set_state(interrupt_speech=False), 0.05
            )
