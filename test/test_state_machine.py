...
def test_none_value_for_add(state_machine):
    key: str = "test"
    test_state: MockState = None
    state_machine.add(key, test_state)
    assert state_machine.states[key] == test_state

def test_valid_value_for_remove(state_machine):
    key: str = "test"
    test_state: MockState = MockState()
...
def test_invalid_value_for_remove(state_machine):
    key: str = "test"
    test_state: MockState = MockState()
    state_machine.states[key] = test_state
...
def test_size(state_machine):
    state_machine.states["test1"] = MockState()
    state_machine.states["test2"] = MockState()
    state_machine.states["test3"] = MockState()
    assert state_machine.size() == 3

def test_valid_change(state_machine):
...
def test_invalid_change(state_machine):
    with pytest.raises(KeyError):
        state_machine.change("abcdefg")

def test_exit_block_change(state_machine):
...
def test_setup_block_change(state_machine):
    test_state1: MockState = MockState()
...
def test_successful_setup(state_machine):
...
def test_empty_state_setup(state_machine):
