...
def test_sound_pool_loads_real_wav_fixture_with_base_path(
    ...
    fixture_path = (
        repo_root / "examples" / "game" / "audio" / "sfx" / "step_dirt.wav"
    )
    assert fixture_path.is_file()

    sound_pool = SoundPool(base_path=repo_root)

    first = sound_pool.load("examples/game/audio/sfx/step_dirt.wav")
    ...
    assert sound_pool.unload("examples/game/audio/sfx/step_dirt.wav")
