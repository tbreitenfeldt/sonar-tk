...
def test_bind_keys_without_escape(
    ...
    assert key_handler_add_key_press_mock.call_args_list[0][0][0]() is True
    assert key_handler_add_key_press_mock.call_args_list[1][0][0]() is True
...
def test_setup(mocker: MockerFixture, default_window: Window):
    ...
    assert (
        starting_state.state_key
        != default_window.state_machine.current_state.state_key
    )
